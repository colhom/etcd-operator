...
var (
	...
	peerServerSecretName, clientServerSecretName string
...
func init() {
	...
	flag.StringVar(&clientServerSecretName, "client-secret-name", "etcd-operator-client", "name of the generated etcd client identity secret")
	flag.StringVar(&peerServerSecretName, "peer-secret-name", "etcd-operator-peer", "name of the generated etcd peer server-identity secret")
	...
	nodePeerServerCfg.DNSNames = []string{fmt.Sprintf("*.%s.%s.svc.cluster.local", clusterName, clusterNamespace)}
...
func main() {
	if clusterName == "" {
		logrus.Fatalf("--cluster-name not provided")
	}
	if clusterNamespace == "" {
		logrus.Fatalf("--cluster-namespace not provided")
	}
	...
	peerSecret, err := k8sutil.NewPeerServerSecret(peerServerSecretName,
		nodePeerKey, nodePeerCert, peerCACert)
	if err != nil {
		logrus.Fatalf("error generating etcd peer identity secret: %v", err)
	}

	// etcd client
	clientSecret, err := k8sutil.NewClientServerSecret(clientServerSecretName,
		nodeClientKey, nodeClientCert, clientCACert)
	if err != nil {
	...
	logrus.Infof("## WRITING RAW PEM FILES ##\n")
	...
	jsonManifests := []jsonManifestFile{
		{
			name: peerServerSecretName + ".json",
			obj:  peerSecret,
		}, {
			name: clientServerSecretName + ".json",
			obj:  clientSecret,
		},
	}

	logrus.Infof("## WRITING KUBERNETES SECRETS ##\n")
